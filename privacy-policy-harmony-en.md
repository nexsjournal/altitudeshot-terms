...
- **Location and altitude information**: After you grant location permission (including precise location and, where needed, approximate location), we use **Location Kit** to read your device's GPS coordinates, altitude, vertical accuracy, heading (for the compass), and related data to display real-time altitude, resolve place names, generate summit/track cards, record track routes, and display the Summit Passport
- **Track route data**: When you use track recording, we continuously record GPS coordinate sequences, distance, duration, speed, and other activity metrics, and save route JSON files locally; **while track recording is in progress**, and after you authorize background location, the App may continue to obtain location updates (which may be accompanied by a system notification or ongoing-task indicator)
- **Photos and images**: When you use the summit camera (**Camera Kit**) or mark an arrival with a one-tap theme background Pin, the App captures or generates images; summit cards and track cards may include your photos plus overlaid text and graphics
...
- **Track record data**: Including end time, start/end/route coordinates, distance, duration, average speed, end-point altitude, card image path, route file path, sport type (hike/run/ride), and template identifier
...
- **Version update detection information**: The App may send app identifier information to public AppGallery query interfaces to obtain the latest version number and determine whether an update prompt is needed; this process does not involve your personal identity or business data
- **Subscription entitlement status**: When you purchase Pro membership, Huawei AppGallery verifies transactions and subscription status through **IAP Kit**; we only read verification results locally on your device to unlock features and **do not collect or store your payment card numbers or other financial information ourselves**
...
- **Reverse geocoding**: To convert coordinates into place names, latitude and longitude may be sent to geocoding capabilities provided by Location Kit or Huawei/system services; this processing is governed by the relevant service provider's privacy policy
- **Weather queries**: Weather conditions are obtained through third-party weather data services such as **Open-Meteo** (this App does **not** use Apple WeatherKit); requests are initiated directly by your device, typically containing only approximate latitude and longitude and necessary query parameters, and we do not retain those requests on our servers
- **Map services**: Track maps, Summit Passport, and related features use Huawei **Map Kit**; during map loading your device may communicate with Map Kit services (such as map tile requests), which may involve approximate location information
...
- **Summit camera and card generation**: Combining photos or theme backgrounds with altitude, place, pressure, time, and other information into shareable cards (the free tier may include a watermark; Pro can remove it)
...
- **Export and sharing**: Saving cards to the system photo library/media library when authorized, or letting you choose recipients through the system share sheet
- **Settings and units**: Remembering your units, language, and related preferences
- **Pro membership benefits**: Unlocking corresponding features based on Huawei IAP verification results
...
- **Feature improvement**: Understanding feature usage (if anonymous analytics or crash services are integrated in the future, this policy will be updated accordingly)
- **Bug fixes**: Collecting necessary diagnostic information to resolve defects quickly
- **Version update prompts**: Comparing AppGallery version numbers and prompting you to update when appropriate
...
- **Summit/track card images** are saved in the app sandbox/private storage and/or in the system photo library if you choose to save them there
...
If you enable device system backup (such as Huawei account cloud backup or manufacturer backup), data in the app-private directory may be included in the corresponding cloud backup service and is governed by that provider's policy. We **do not operate** a separate cloud business database; such backups are managed by you and your operating system provider.
...
#### 4.2.1 Huawei / HarmonyOS System and Kit Services

- **Location Kit**
  - **Purpose**: Obtain GPS altitude, coordinates, heading, and location permission status; background location updates may be enabled while track recording is in progress (such as `LOCATION_IN_BACKGROUND`; exact permission names follow the system)
  - **Data processing**: Processed locally on your device after authorization; reverse geocoding and similar requests may be handled by Huawei-related services
  - **Your control**: You can manage location permissions (including background location) for this App in system Settings

- **Map Kit**
  - **Purpose**: Track maps, Summit Passport map display, route visualization, and map snapshots
  - **Data processing**: Map tiles and related requests are handled by Huawei Map Kit services and may involve approximate location information
  - **Details**: Please see the [Huawei Consumer Business Privacy Statement](https://consumer.huawei.com/en/privacy/privacy-policy/)
...
- **Media library / photo library write access**
  - **Purpose**: Save generated summit/track cards to the system photo library or media library
  - **Data processing**: Files are written directly to your device; the first save may require a dynamic permission request
  - **Your control**: You can revoke related media write permissions at any time

- **Sensor services (barometer / orientation, etc.)**
  - **Purpose**: Read pressure, compass direction, and related data on supported devices
  - **Data processing**: Processed locally on the device; related UI is hidden on devices without a barometer

- **Background location and notifications / ongoing tasks**
  - **Purpose**: Maintain continuous location while track recording is in progress and possibly show a recording-status indicator
  - **Data processing**: Indicator content is used only to show recording status and does not include your full track details; background location stops immediately when recording ends
...
- **Geocoding**
  ...
  - **Data processing**: Prefer reverse geocoding from Location Kit / system capabilities; when unavailable, the App may fall back to displaying coordinate strings

#### 4.2.2 Third-Party Weather Data (Open-Meteo)
...
- **Note**: When the network is unavailable, the App may fall back to local rules for display without sending requests; Settings attributes weather data to Open-Meteo

#### 4.2.3 App Store Services

- **Huawei AppGallery / AppGallery Connect**
  - **Purpose**: App distribution, version update queries, in-app purchases, and subscription management
  - **Data processing**: Handled by Huawei under its privacy policy
...
- **Local encryption and sandboxing**: Relying on HarmonyOS data protection mechanisms, app data is stored in private spaces where other apps cannot access it directly
- **Minimum permissions**: Requesting only necessary permissions such as location, camera, photo library/media, and network when related features are used; background location is requested only when you start track recording
...
- **Secure development practices**: Following Huawei / HarmonyOS security recommendations and addressing known issues promptly
...
- Grant app permissions carefully and review this App's permission list regularly in system Settings
...
- **Delete data**: Delete individual records through in-app actions; deletion removes local database entries and associated image/route files
...
You can revoke app permissions at any time in your device system settings. You can usually find AltitudeNow in Settings and manage location (including background location), camera, photo library/media, notifications, and related permissions. Track recording may require separate authorization for background location. Revoking permissions may make related features unavailable or degraded (for example, without location permission, real-time altitude or track recording cannot work).
...
- Map Kit, IAP, and other Huawei services may be unavailable in some regions; related features may be hidden or degraded when unavailable
...
- Changes in app features (such as subscriptions, analytics services, cross-device sync, etc.)
...
- Describe the changes in the AppGallery release notes
